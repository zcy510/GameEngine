...
#include "Taro/Core/AssetsManager.h"
...
namespace Taro {
	...
	void Model::LoadModel(const std::string& path)
	{
		...
		{
			TR_CORE_ERROR("ASSIMP ERROR: {0}", importer.GetErrorString());
		}
	...
	Ref<Material> Model::LoadMaterial(aiMaterial* material)
	{
		...
		for (uint32_t i = 0; i < numTexTypes; i++)
		{
			aiTextureType aiType = aiTypes[i];
			Texture::Type type = types[i];

			for (uint32_t j = 0; j < material->GetTextureCount(aiType); j++)
			{
				aiString str;
				material->GetTexture(aiType, j, &str);
				...
				result->SetTextureMap(texture, type);
			}
		}
